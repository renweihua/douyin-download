...
class Video extends Server
{
            ...
            if (isset($response['aweme_list'])) {
                ...
                if ($response['aweme_list'] && empty($author)){
                    ...
                    if ($open_download){
                        ...
                        $path_file_folder .= '/' . $author['nick_name'] . '-' .  $this->getFileFolder($author['unique_id']);
                ...
                if (isset($author['nick_name']) && !empty($author['nick_name'])){
                    $author['nick_name'] = $response['aweme_list'][0]['author']['nickname'] ?? '';
                }
                // 是否还有更多数据
                ...
                foreach ($response['aweme_list'] as $item){
                    ...
                    // 开启下载，下载文件
                    if ($open_download){
                        $file_name = empty($video['desc']) ? $video['aweme_id'] : $video['desc'];
                        ...
                        $video['real_video_path'] = $path_file_name;
                    }
    ...
    private function getFileFolder(string $string): string
    {
